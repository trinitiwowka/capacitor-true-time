...
struct TrueTimeSample {
    ...
    let delay: Double
    let offset: Double
    let rootDelay: Double
    let rootDispersion: Double
    let stratum: UInt8
    let leap: UInt8
...
final class SNTPClient {
    ...
    static let maxRootDelayMs = 100.0
    static let maxRootDispersionMs = 100.0
    static let maxRoundTripDelayMs = 750.0
    ...
    private func sendRequest() {
        ...
        let request = Self.makeRequest(date: requestDate)
    ...
    static func parseResponse(request: Data, response: Data, t0: Double, t3: Double) throws -> TrueTimeSample {
        ...
        let header = try validateHeader(bytes)
        ...
        return TrueTimeSample(
            ...
            delay: delay,
            offset: offset,
            rootDelay: header.rootDelay,
            rootDispersion: header.rootDispersion,
            stratum: header.stratum,
            leap: header.leap
        )
    }

    private static func validateHeader(_ bytes: [UInt8]) throws -> SNTPHeader {
        let leap = bytes[0] >> 6
        let version = (bytes[0] >> 3) & 0x07
        let mode = bytes[0] & 0x07
        let stratum = bytes[1]
        let rootDelay = readSignedShortFixed(bytes, offset: 4)
        let rootDispersion = readUnsignedShortFixed(bytes, offset: 8)

        guard leap != 3 else { throw SNTPError.malformed("server clock is unsynchronized") }
        guard version >= 3 && version <= 4 else { throw SNTPError.malformed("unsupported protocol version") }
        guard mode == 4 else { throw SNTPError.malformed("response is not in server mode") }
        guard stratum >= 1 && stratum <= 15 else { throw SNTPError.malformed("invalid stratum") }
        guard rootDelay <= maxRootDelayMs else { throw SNTPError.malformed("root delay is too large") }
        guard rootDispersion <= maxRootDispersionMs else { throw SNTPError.malformed("root dispersion is too large") }
    ...
    private static func readSignedShortFixed(_ bytes: [UInt8], offset: Int) -> Double {
        Double(Int32(bitPattern: readUInt32(bytes, offset: offset))) / 65.536
    }

    private static func readUnsignedShortFixed(_ bytes: [UInt8], offset: Int) -> Double {
        Double(readUInt32(bytes, offset: offset)) / 65.536
    }
